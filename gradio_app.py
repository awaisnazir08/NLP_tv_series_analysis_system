...
def get_themes(theme_list_str, subtitles_path, save_path):
    theme_list_str = theme_list_str.replace("'", '')
    theme_list_str = theme_list_str.replace('"', '')
    theme_list = theme_list_str.split(',')
    theme_classifier = ThemeClassifier(theme_list)
    output_df = theme_classifier.get_themes(subtitles_path, save_path)
    # Remove the dialogue from the themes list
    ...
    output_chart = gr.BarPlot(output_df, x='Theme', y='Score', title = 'Series Themes', tooltip=['Theme', 'Score'], vertical=False, width=500, height=260)
...
def main():
    with gr.Blocks() as iface:
            ...
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        get_networks_graph_button = gr.Button('Get Character Network')
                        get_networks_graph_button.click(get_character_network, inputs=[subtitles_path, ner_path], outputs = [network_html])
        ...
        with gr.Row():
            with gr.Column():
                gr.HTML("<h1>Character Chatbot based on Naruto</h1>")
